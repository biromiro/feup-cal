...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    ...
    auto src = findVertex(source), snk = findVertex(sink);

    resetFlows();
    bellmanFordShortestPath(src);
    reduceCost();
    double result = 0;
    findAugmentationPath(src, snk);
    while(result != flow ){
        augmentFlowAlongPath(src, snk, findMinResidualAlongPath(src, snk));
        dijkstraShortestPath(src);
        reduceCost();
        result = getFlow(source, sink);
    }
    return snk->dist;
}

template <class T>
void Graph<T>::reduceCost(){
    for(Vertex<T>* vertex: vertexSet){
        for(Edge<T>* edge: vertex->getOutgoing()){
            edge->cost += vertex->dist - edge->dest->dist;
        }
    }
}
