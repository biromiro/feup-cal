...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    //loop invariant -> res contains the minimally distanced pair starting at one of vp[0:i]'s points (by Balta)
    //loop variant ->n =  vp.size(), (n - i) -> strictly decreasing and when it reaches 1, i=n-1 and vp[0:n-1]
    // contains the minimum distance of all points in vp
}
...
Result minDist(const Result& d1, const Result& d2){
    if(d1.dmin < d2.dmin) return d1;
    else return d2;
}

Result nearestPoints_DC_R(std::vector<Point> &vp){
    if(vp.empty() || vp.size() == 1) return {};
    if(vp.size() == 2) return {vp[0].distance(vp[1]), vp[0], vp[1]};
    std::vector<Point> left = std::vector<Point>(vp.begin(),vp.begin() + vp.size()/2);
    std::vector<Point> right = std::vector<Point>(vp.begin() + vp.size()/2,vp.end());
    Result dL = nearestPoints_DC_R(left), dR = nearestPoints_DC_R(right), delta = minDist(dL, dR);
    double divisionLineX = (left.back().x + right.front().x) / 2;
    std::vector<Point> inStrip;
    for(auto i = left.rbegin(); i != left.rend(); i++) if(divisionLineX - i->x < delta.dmin) inStrip.push_back(*i);
    for(auto & i : right) if(i.x - divisionLineX < delta.dmin) inStrip.push_back(i);

    sortByY(inStrip, 0, inStrip.size() - 1);
    sortByY(inStrip, 0, inStrip.size() - 1);

    double distance;
    for(size_t i = 0; i < inStrip.size(); ++i){
        for(size_t j = i + 1; j < inStrip.size(); ++j){
            if(std::abs(inStrip[i].y - inStrip[j].y) >= delta.dmin) break;
            else{
                if((distance = inStrip[i].distance(inStrip[j])) < delta.dmin) delta = Result(distance, inStrip[i], inStrip[j]);
            }
        }
    }
...
Result nearestPoints_DC_MT_R(std::vector<Point> &vp, int threads){
    if(vp.empty() || vp.size() == 1) return {};
    if(vp.size() == 2) return {vp[0].distance(vp[1]), vp[0], vp[1]};
    std::vector<Point> left = std::vector<Point>(vp.begin(),vp.begin() + vp.size()/2);
    std::vector<Point> right = std::vector<Point>(vp.begin() + vp.size()/2,vp.end());
    Result dL, dR;
    if(threads > 1){
        std::thread t([&](){dL = nearestPoints_DC_MT_R(left, threads/2);});
        dR = nearestPoints_DC_MT_R(right, threads/2);
        t.join();
    }else{
        dL = nearestPoints_DC_R(left), dR = nearestPoints_DC_R(right);
    }
    Result delta = minDist(dL, dR);
    double divisionLineX = (left.back().x + right.front().x) / 2;
    std::vector<Point> inStrip;
    for(auto i = left.rbegin(); i != left.rend(); i++) if(divisionLineX - i->x < delta.dmin) inStrip.push_back(*i);
    for(auto & i : right) if(i.x - divisionLineX < delta.dmin) inStrip.push_back(i);
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    sortByX(vp, 0, vp.size()-1);
    return nearestPoints_DC_R(vp);

    //to sort -> O(nlogn), each division is O(logn) => O(nlogn)*O(logn) = O(nlog^2(n))
}

Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    Result res;
    sortByX(vp, 0, vp.size()-1);
    return nearestPoints_DC_MT_R(vp,numThreads);
}
...
TEST(TP3_Ex1, testNP_DC_Max) {
    unsigned int numThreads = std::thread::hardware_concurrency();
    std::cout << numThreads << std::endl;
    setNumThreads(numThreads);
    testNearestPoints(nearestPoints_DC_MT, "Divide and conquer with max threads");
}